function dt6FileName = dtiRawFit_Charmed_highB_comp(dwRaw, bvecs, bvals, outBaseName, bs, fitMethod, adcUnits, xformToAcPc,ROICLmask,BPF,Gmcc,Wmcc,Cmcc,dt6_path,hind,restric)
%
...
% fields:
%Par.F_h=hind; %each hindered has to 4 param (2 l and 2 angel)
% 
% Par.F_R=restric;%each restricted has to 3 param (1 l and 2 angel)
% 
...
x0=zeros(4*Par.F_h+3*Par.F_R+Par.F_h+Par.F_R,1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for(ii=1:nvox)
            ...
            %initiate fractions:
           num= Par.F_h+Par.F_R-1;
           
           x0(1+last_var:last_var+num)=1/(Par.F_h+Par.F_R); %the comparttment are h(:) then r-1 compartment;
           %initiate noise
           x0(1+last_var+num)=0.03; % this is a case from yaniv data
           
        
            [x1, resnorm] = lsqnonlin(@(x) Charmed_Mconp_err(x,raw,Par),x0,lb,ub,options);
