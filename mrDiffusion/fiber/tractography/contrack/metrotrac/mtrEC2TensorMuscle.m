function mtrEC2TensorMuscle(subjectDirName, numRepeats, diffusionTime, numBootstrap)
...
rawAlignedFile = fullfile('raw','rawDtiAligned.nii.gz');
rawAlignedVoxelFormatFile = fullfile('raw','rawDtiAligned.Bfloat');
rawBvecsFile = fullfile('raw','rawDtiAligned.bvecs');
...
if bGenScheme
    ...
    delete(schemeFile);
    cmd = sprintf('fsl2scheme -bvalfile raw/bvals -bvecfile %s -diffusiontime %g -outputfile %s', rawBvecsFile, diffusionTime, schemeFile);
    disp(cmd);
...
dtiWriteNiftiWrapper(uint8(ones(size(img_bm))),xformToAcPc,fullfile('bin','brainMask.nii.gz'));
...
if bUseT1
    ...
    % were not closed
    img_wm = img_bm & img_fa>.15;
    img_wm = dtiCleanImageMask(img_wm,0,1);    
