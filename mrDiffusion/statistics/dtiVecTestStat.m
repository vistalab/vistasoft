function [T, M, S, DISTR, df] = dtiVecTestStat(g1, g2, Y, mask)

% Computes voxel-wise statistics for two groups from a data array of
% diffusion tensors in dt6 format.
%
% The test is H0: both groups have the same eigenvectors, but common
% unknown eigenvalues.
%
%   [T, M, S, DISTR, df] = dtiVecTestStat(g1, g2, DT_ARRAY, [MASK])
%
% Input:
...
S1 = sum(sum(d1(:,:,:,1:p,:).^2, 4) + 2*sum(d1(:,:,:,p+1:q,:).^2, 4),5);
S2 = sum(sum(d2(:,:,:,1:p,:).^2, 4) + 2*sum(d2(:,:,:,p+1:q,:).^2, 4),5);
% S = (sum(S, 5) + N1*N2/N * sum((L1 - L2).^2,4))/(q*N);
S = (S1+S2)/(q*(N-2));
clear S1 S2;

% Chi2 version
% DISTR = 'chi2';
% df = [q-p, 0];   % Second df is dummy
% T = N * sum(((N1*L1 + N2*L2)/N).^2 - L.^2, 4) ./ S;

% F version
DISTR = 'f';
% df = [q-p, q*(N-2)+p];
df = [q-p, q*(N-2)];
T = sum(((N1*L1 + N2*L2)/N).^2 - L.^2, 4);
% T = df(2)/df(1) * T./S / q;
