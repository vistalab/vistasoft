...
% I hate to contribute to this tangle of code without cleaning it up,
% but I must...
[vAnatName,vAnatPath] = uigetfile('*.dat', 'Select subject''s vAnatomy file');
...
uimenu(ld,'Label','Load Volume Anatomy','CallBack', ...
    '[volume, sagSize, numSlices, calc, dataRange] = mrLoadVAnatomy([vAnatPath,vAnatName]); curSag = floor(numSlices/2); sagSlice = mrShowSagVol(volume,sagSize,curSag,[]);','Separator','on');
...
if ~check4File('AlignParams') & ~check4File('VolParams')
  mrGetAlignParams3(vAnatPath,mrSESSION.subject); 
end
...
[volume_pix_size] = mrGetVolPixSize(vAnatPath);
...
[volume, sagSize, numSlices, calc, dataRange] = mrLoadVAnatomy([vAnatPath,vAnatName]);
