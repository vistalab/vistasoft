%% this script makes draws a contour of a coverage map
% of a specified normalized value (between 0 and 1)
% if bootstrapping, can also draw shaded standard error bars around the
% contour
%
% rl, 08/14
...
% contour level we want to see (value between 0 and 1)
tem.conLevel = 0.9; 
...
tem.saveFolderSingle = ['/biac4/wandell/data/reading_prf/forAnalysis/images/single/contours/withError/' tem.saveFolderSub '/'];
tem.saveFolderGroup  = ['/biac4/wandell/data/reading_prf/forAnalysis/images/group/contours/withError/' tem.saveFolderSub '/']; 
    ...
    if tem.YNthreshold
        ...
        for ii = 1:length(tem.subjectsToAnalyze)
            ...
            if tem.passesThreshold
                ...
                [tem.handleContour] = ff_pRFasContoursWithErrorBars(this.RFcov, [tem.conLevel tem.conLevel]); 
    ...
    switch tem.YNgroupAverage
        ...
        case 1
            ...
            if exist('RFcov','var')
                ...
                tem.figHandleBoot = ff_pRFasContoursWithErrorBars(mean(boot.allSamples,3), [tem.conLevel tem.conLevel], boot.allSamples);
