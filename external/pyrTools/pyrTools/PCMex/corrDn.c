...
void mexFunction(int nlhs,	     /* Num return vals on lhs */
  ...
  /* ARG 1: IMAGE  */
  arg = prhs[0];
  if notDblMtx(arg) mexErrMsgTxt("IMAGE arg must be a non-sparse double float mxArray.");
  image = mxGetPr(arg);
  ...
  /* ARG 2: FILTER */
  arg = prhs[1];
  if notDblMtx(arg) mexErrMsgTxt("FILTER arg must be non-sparse double float mxArray.");
  filt = mxGetPr(arg);
  ...
  if (nrhs>3)
      {
      arg = prhs[3];
      if notDblMtx(arg) mexErrMsgTxt("STEP arg must be a double float mxArray.");
  ...
  if (nrhs>4)
      {
      const arg = prhs[4];
      if notDblMtx(arg) mexErrMsgTxt("START arg must be a double float mxArray.");
      if (mxGetM(arg) * mxGetN(arg) != 2)
  ...
  if (nrhs>5)
      {
      if notDblMtx(prhs[5]) mexErrMsgTxt("STOP arg must be double float mxArray.");
  ...
  if (plhs[0] == NULL) mexErrMsgTxt("Cannot allocate result mxArray");
