...
%%% You should go through this, reading the comments, and executing the
%%% corresponding MatLab instructions.  This file assumes a basic
%%% familiarity with matrix algebra, with linear systems and Fourier
%%% theory, and with MatLab.  If you don't understand a particular
%%% function call, execute "help <functionName>" to see documentation.
%%%
%%% EPS, 6/96.  
%%% Based on the original OBVIUS tutorial by DJH/EPS.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% filter is flipped) and downsamples in a single operation (for
...
%% full size before subtracting it from the original.  The upConv function does
%% upsampling (padding with zeros between samples) followed by convolution.
%% This can be done using the lowpass filter that was applied before
...
%% (blurred1 and fine1) containing the coarse scale information and
%% the fine scale information.  We can also (trivially) reconstruct the
%% original from these two (even if the subsampling of the blurred1 image caused 
%% aliasing):
...
%% MATLAB uses 5-tap filters by default for building Laplacian
...
%% transform is overcomplete by a factor of 4/3, and the
%% 2-dimensional transform is overcomplete by a factor of 2.
%% Secondly, the "bandpass" images (fineN) do not segregate
%% information according to orientation.

%% There are other varieties of pyramid.  One type that arose in the
%% speech coding community is based on a particular type of filter
%% known as a "Quadrature Mirror Filter" or QMF.  These are closely
%% related to Wavelets.
...
%% pyramid, we apply two filters (hi- and lo- pass) and subsample
%% BOTH by a factor of 2, thus eliminating the excess coefficients of
...
%% The reconWpyr method can be used to reconstruct the entire pyramid:
...
%% by multiples of two are also trivially orthogonal.  Here we
%% construct the matrix of projection functions, and show that it is
%% orthonormal:
...
%% If you want perfect reconstruction, there's another option: drop the
%% symmetry requirement.  Ingrid Daubechies developed one of the earliest sets
%% of such perfect-reconstruction wavelets.  The simplest of these is of length
%% 4:
daub_lo = namedFilter('daub2');
daub_hi = modulateFlip(daub_lo);

%% These are used in the same system diagram as the QMF filters.  To
%% see orthogonality, construct the matrix of projection functions:
...
%% In the third band, we expected the coefficients to move around.  But notice
%% that in the original signal decomposition, the first two bands are zero.
%% AFter the shift, they have significant content.  Although these subbands are
%% supposed to represent information at different scales, their content also
%% depends on the relative POSITION of the input signal.
...
%% for almost any orthogonal pyramid transform (the only exception is the
%% limiting case in which the filter is a sinc function).
...
%% (like the QMF/Wavelet transform).
...
%% using the filters {fl1,fb0,fb1,...,fbK}).
