...
%          Can be an INT a DECIMAL or a STRING:
...
%           STRING - use this mode when starting the pool. Options are:
...
if isnumeric(cpus) && cpus ~=0
    ...
    if cpus <= total_cpus
        ...
        disp('Total number of CPUS is less than desired. Setting to max.');
        numcpus = total_cpus;
    end
elseif cpus == 0
    % Set to max
