...
public:
    KdTree(float destPoints[][3], int numDestPoints);
    ~KdTree();
    void getNearestPoint(const KdPoint &target,
                         int *bestIndex, float *bestSqDistance);
private:
 ...
 */
KdTree::KdTree(float destPoints[][3], int numDestPoints)
{
                            ...
                            int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 2) {
    ...
    } else if (!mxIsSingle(prhs[0]) || mxIsComplex(prhs[0]) || mxGetNumberOfDimensions(prhs[0]) != 2) {
        mexErrMsgTxt("First arg must be a single precision 3xM array.");
    } else if (!mxIsSingle(prhs[1]) || mxIsComplex(prhs[1]) || mxGetNumberOfDimensions(prhs[1]) != 2) {
        mexErrMsgTxt("Second arg must be a single precision 3xN array.");
    } else {
        /* Input matrices */
        const mwSize *srcDims      = mxGetDimensions(prhs[0]);
        float (*srcPtr)[3]  = (float(*)[3])mxGetPr(prhs[0]);
        const mwSize *destDims     = mxGetDimensions(prhs[1]);
        float (*destPtr)[3] = (float (*)[3])mxGetPr(prhs[1]);
        ...
        sqDist = (float *)mxMalloc(srcDims[1] * sizeof(float));
