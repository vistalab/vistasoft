function [tMat, hdr] = readMagFile(magFile,slice,useLittleEndian);
%
% [tMat, hdr] = readMagFile(magFile,[slice],[useLittleEndian]);
%
% Read a P*.mag file into a 4D tSeries matrix, 
% without any of the built-in assumptions of mrInitRet.
% Requires that a corresponding E-file header be
% present. If the second argument, slice, is entered,
% only returns the tSeries for that slice; otherwise,
% returns the entire 4D array in the format:
% rows, cols, slices, frames.
%
