...
if ~isfield(params, 'diameter'), params.diameter = 256; end
...
% for hemifield wedges, squeeze the wedge values into half the field
if ismember(params.visualField, {'l' 'r'})
	wedge = mod(2 * atan2(Y, X), 2*pi);
end
...
switch params.visualField
    ...
    otherwise   
        if isnumeric(params.visualField)    % extent of colors, in degrees
            % ? Not sure yet how to handle this -- probably should happen
            % earlier ?
        end
end
