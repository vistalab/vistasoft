function prediction = rmHrfSearchFit_oneGaussianCSS(model, params, loopSlices, wProcess,varexp,allstimimages)
% rmHrfSearchFit_oneGaussian - make predictions without HRF
%
% prediction = rmHrfSearchFit_oneGaussianCSS(model, params, loopSlices, wProcess)
%
...
for slice=loopSlices,
    ...
    tmp.n = [tmp.s s{1}.exponent(wProcessSlice)];
...
for n=1:numel(s)-1,
    ...
    pred = (allstimimages*rf).^ tmp.n(s(n):s(n+1)-1);
