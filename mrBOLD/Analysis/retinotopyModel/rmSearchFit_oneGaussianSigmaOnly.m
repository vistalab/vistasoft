function model = rmSearchFit_oneGaussianSigmaOnly(model, data, params, wProcess, t)
...
for ii = 1:numel(wProcess),
    ...
    if searchOptions.MaxIter>0
        outParams = ...
            fmincon(@(x) rmModelSearchFit_oneGaussianSigmaOnly(x,vData,XvYv,...
            params.analysis.allstimimages,trends),...
            range.start(3,vi),[],[],[],[],range.lower(3,vi),range.upper(3,vi),...
            [],searchOptions);        
    else
        outParams = range.start(3,vi);
    end
    %[ outParams range.lower(3,vi) range.start(3,vi) range.upper(3,vi)]

    % make RF, prediction and get rss,b
    rf = exp( XvYv ./ (-2.*(outParams.^2)) );
    ...
    % fit. 
    if b(1)>0,
        model.s(vi)          = outParams;
        model.s_major(vi)    = outParams;
        model.s_minor(vi)    = outParams;
        model.s_theta(vi)    = 0;
        model.rss(vi)  = rss;
        model.b([1 t_id],vi)  = b;
    else
