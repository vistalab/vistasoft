function model = rmSearchFit_oneOvalGaussianNoTheta(model, data, params, wProcess, t)
% rmSearchFit_oneOvalGaussian - wrapper for 'fine' one oval Gaussian fit
% without theta
%
% model = rmSearchFit_oneOvalGaussianNoTheta(model, data, params, wProcess, t);
%
...
for ii = 1:numel(wProcess),
    ...
    startParams = [model.x0(vi); model.y0(vi); model.s_major(vi); model.s_minor(vi)];
    ...
    bndParams          = [boundary.xy;boundary.sigma;boundary.sigma;];

    % actual fitting routine
    if searchOptions.MaxIter>0
        outParams = ...
            fmincon(@(x) rmModelSearchFit_oneOvalGaussianNoTheta(x,vData,...
            params.analysis.X,...
            params.analysis.Y,...
    ...
    if outParams(1)==0 && outParams(2)==0
        theta=0;
    else
        theta = -atan(outParams(2)/outParams(1));
    end    
