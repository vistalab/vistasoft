function fmri_loc2_GLMcontrasts
...
%
% Calls the functions:
% * fmri_loc2_setGLMparams (subfunction)
% * fmri_loc2_checkDataTypesForGLM
% * fmri_loc2_computeContrastsLocalizer
%
%
% DY 06/05/2008
...
% AL 11/08 Modified for loc2 GLM
...
logFile = fullfile(fmriDir,'logs',['LO2_Log_' dateAndTime '.txt']);
fid=fopen(logFile,'w');
[junk,logFileName]=fileparts(logFile)
scriptName = 'fmri_loc2_GLMcontrasts.m';
startTime = clock;

% 1. Set GLM parameters
params=fmri_loc2_setGLMparams(fid,logFileName,scriptName); % Subfunction within this script
...
for ii=1:length(subs)
    ...
    theglm=find(strcmp({dataTYPES.name},'GLMs'));
     if ~isempty(theglm)
        ...
        alreadyDone=[];
        for xx=1:checkScans
            tmp=(strfind(lower(dataTYPES(theglm).eventAnalysisParams(xx).annotation),'lo localizer run 2'));
            alreadyDone=[alreadyDone tmp];
        end
        if(~isempty(alreadyDone))
            fprintf(fid,'Error: Subject already processed (has GLM with ''LO localizer Run 2'' in annotation field), skipping... \n\n');
            fprintf('Error: Subject already processed (has GLM with ''LO localizer Run 2'' in annotation field), skipping... \n\n');
            continue
        end
    end
    ...
    [go,s2]=fmri_loc2_checkDataTypesForGLM(fid,dataTYPES);
    ...
    elseif go==1
        try
            ...
            er_setParams(hI, params, s2.scan, s2.mc); % write params as eventAnalysisParams fields
            tic; [hI, lo_scan] = applyGlm(hI,s2.mc, s2.scangroup,params);
            time=toc;
        ...
        if glmOK
            ...
            try
                fmri_loc2_computeContrastsLocalizer(hI, lo_scan)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function params=fmri_loc2_setGLMparams(fid,logFileName,scriptName)
...
p.an='params.annotation=''LO localizer Run 2'''; eval(p.an); fprintf(fid,'%s \n',p.an);
